...
def get_signal_strengths(wifi_scan_method):
    ...
    # Mac OS X
        ...
        if airport_scan_status != 0:
            ...
            #     print(network.findall("string")[1].text)

            wifi_data = [(network.findall("string")[0].text, abs(int(network.findall("integer")[7].text)), network.findall("string")[1].text) for network in networks]
            # input()
...
def getWifi():
    ...
    location_request = {
        ...
        'wifiAccessPoints':[
            ...
            } for mac, signal, name in wifi_data]
    }
